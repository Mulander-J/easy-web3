...
// console.log('[ process.env.REACT_APP_CANDY_MACHINE_CONFIG ] >', process.env.REACT_APP_CANDY_MACHINE_CONFIG)
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        if (!mints.find((mint) => mint.image === parse.image)) {
          setMints((prevState) => [...prevState, {image:parse.image, _data: mint.data}]);
        }
      }
    }
    // Set loading flag.
      ...
      <div className="gif-grid">
        {mints.map((mint,i) => (
          <div className="gif-item" key={i}>
            <img src={mint.image} alt={`Minted NFT ${mint._data.name}`} />
            <p>{mint._data.name}</p>
          </div>
        ))}
      </div>
